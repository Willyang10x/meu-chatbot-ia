...
export default function Home() {
  ...
  const enviarMensagem = async (e?: React.FormEvent, textoDireto?: string) => {
    ...
    const novaMensagemUsuario: Mensagem = { 
      autor: "usuario", 
      texto: textoMensagem + (usarInternet ? "\n\n*[🌐 Pesquisa Web Ativada]*" : ""),
    ...
    const pesquisaAtiva = usarInternet;
    limparImagem();
    limparPdf();
    setUsarInternet(false);
  ...
  const regerarUltimaResposta = async () => {
    ...
    const usouInternetAntes = ultimaMsgUsuario.texto.includes("*[🌐 Pesquisa Web Ativada]*");
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl px-2">
                {sugestoes.map((sugestao, i) => (
                  <motion.button
                    ...
                    key={i}
                    onClick={() => { 
                      if(sugestao.texto.includes("Dólar hoje")) setUsarInternet(true); 
                      enviarMensagem(undefined, sugestao.texto); 
                    }}
                      ...
                      {msg.autor === "ia" ? (
                        <>
                          ...
                          <div className="flex justify-start mt-4 gap-3">
                            <button onClick={() => copiarTexto(msg.texto, index)} className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-300 transition-colors bg-white/5 px-2 py-1.5 rounded-lg">
                              {copiadoIndex === index ? (
                            ...
                            <button onClick={() => alternarVoz(msg.texto, index)} className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-300 transition-colors bg-white/5 px-2 py-1.5 rounded-lg">
                              {falandoIndex === index ? (
                            ...
                            {index === mensagens.length - 1 && !carregando && (
                              <button onClick={regerarUltimaResposta} className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-300 transition-colors bg-white/5 px-2 py-1.5 rounded-lg">
                        ...
                        <div className="flex flex-col gap-2 items-end w-full group">
                          {editandoIndex === index ? (
                            ...
                            <>
                              ...
                              {msg.documento && !msg.texto.includes("📄 PDF anexado") && (
                                <div className={`flex items-center gap-2 p-2 rounded-lg border ${tema.border} bg-black/20 mb-2`}>
                                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                                  <span className="text-xs text-gray-300">Documento PDF</span>
                                </div>
                              )}
                              <p className="whitespace-pre-wrap leading-relaxed text-[15px]">{msg.texto}</p>
                              {!carregando && (
                                <button
                                  onClick={() => iniciarEdicao(index, msg.texto.replace("\n\n*[🌐 Pesquisa Web Ativada]*", ""))}
          ...
          <div className="w-full max-w-4xl relative">
            ...
            <AnimatePresence>
              {imagemPreview && (
                <motion.div initial={{ opacity: 0, scale: 0.8, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8, y: 20 }} className={`absolute bottom-[110%] left-0 bg-black/60 backdrop-blur-md p-2 rounded-2xl border ${tema.border} shadow-2xl inline-block mb-3`}>
                  <button onClick={limparImagem} className="absolute -top-3 -right-3 bg-red-500 hover:bg-red-600 shadow-md transition-colors text-white rounded-full w-7 h-7 flex items-center justify-center text-xs font-bold">✕</button>
              ...
              {pdfNome && (
                <motion.div initial={{ opacity: 0, scale: 0.8, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8, y: 20 }} className={`absolute bottom-[110%] left-0 bg-black/60 backdrop-blur-md px-4 py-3 rounded-2xl border ${tema.border} shadow-2xl inline-flex items-center gap-3 mb-3`}>
                  <button onClick={limparPdf} className="absolute -top-3 -right-3 bg-red-500 hover:bg-red-600 shadow-md transition-colors text-white rounded-full w-7 h-7 flex items-center justify-center text-xs font-bold">✕</button>
            ...
            <form onSubmit={enviarMensagem} className={`relative flex items-center w-full gap-1 sm:gap-2 bg-black/40 rounded-full border ${tema.border} shadow-lg px-2 transition-all focus-within:ring-1 focus-within:ring-white/20`}>
              ...
              <div className="flex items-center">
                <button type="button" onClick={() => setUsarInternet(!usarInternet)} className={`p-2 sm:p-3 rounded-full transition-colors flex-shrink-0 flex items-center justify-center h-10 w-10 sm:h-11 sm:w-11 ${usarInternet ? "bg-blue-500/20 text-blue-400" : "bg-transparent text-gray-400 hover:text-white"}`} disabled={carregando}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg>
                </button>

                <button type="button" onClick={() => pdfInputRef.current?.click()} className="p-2 sm:p-3 rounded-full transition-colors flex-shrink-0 flex items-center justify-center h-10 w-10 sm:h-11 sm:w-11 bg-transparent text-gray-400 hover:text-white" disabled={carregando}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"></path></svg>
                </button>

                <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2 sm:p-3 rounded-full transition-colors flex-shrink-0 flex items-center justify-center h-10 w-10 sm:h-11 sm:w-11 bg-transparent text-gray-400 hover:text-white" disabled={carregando}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
                </button>

                <button type="button" onClick={alternarMicrofone} className={`p-2 sm:p-3 rounded-full transition-colors flex-shrink-0 flex items-center justify-center h-10 w-10 sm:h-11 sm:w-11 ${ouvindo ? "bg-red-500/20 text-red-500 animate-pulse" : "bg-transparent text-gray-400 hover:text-white"}`} disabled={carregando}>
